...
- **preload** (`desktop/src/preload/index.ts`): exposes exactly one bridge, `window.hq`, with `pty`, `app`, and `actions` namespaces. All new IPC must go through the typed `IPC` constants in `desktop/src/shared/ipc.ts` — that file is the single source of truth for channel names and payload types, imported by both sides.
